...
namespace lua {
    ...
    public:
        ...
        /// Constructor for returning nil from functions
        Value(lua::Nil) : _stack(nullptr) {
        }

        /// Returns true if this Value has an associated Lua stack (i.e. is not nil)
        bool hasStack() const {
        ...
        int length() const {
            return static_cast<int>(lua_rawlen(_stack->state, _stack->top + _stack->pushed - _stack->grouped));
        }
