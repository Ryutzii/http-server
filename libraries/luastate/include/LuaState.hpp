...
//            if (!executed)
    ...
    public:
        ...
        void setData(lua::String key, const char* value, size_t length) const {
            stack::push_str(_luaState, value, static_cast<int>(length));
            lua_setglobal(_luaState, key);
        }
        ...
        /// Creates a new empty Lua table and returns it as a lua::Value.
        /// This avoids going through the interpreter to obtain a table value.
        lua::Value newTable() const {
            int stackTop = stack::top(_luaState);
            lua_newtable(_luaState);
            return lua::Value(std::make_shared<detail::StackItem>(_luaState, _deallocQueue, stackTop, 1, 0));
        }
    };
}
